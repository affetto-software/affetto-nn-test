...
OUTPUT_BASE="$HOME/Dropbox/develop/data/20220719/predict_ctrl_input_linear"
  ...
  for j in "${JOINT[@]}"; do
    for k in "${N_PREDICT[@]}"; do
      for s in "${SCALER[@]}"; do
        ...
        cmd=(pdm run ./apps/predict_ctrl_input_linear.py --train-data "${TRAIN_DATA_SET_A[@]}" --test-data "${i}" --joint "${j}" --n-predict "${k}" --scaler "${s}" -d "${OUTPUT_BASE}"/SET-A_joint-"$j"_n-predict-"$k"_scaler-"$s"-"${TEST_TRAJ_LABELS[$ii]}" -e png -s -x)
        echo "${cmd[@]}" >>"$LOG"
        "${cmd[@]}" >>"$LOG" 2>&1
      done
    done
  ...
  for j in "${JOINT[@]}"; do
    for k in "${N_PREDICT[@]}"; do
      for s in "${SCALER[@]}"; do
        ...
        cmd=(pdm run ./apps/predict_ctrl_input_linear.py --train-data "${TRAIN_DATA_SET_B[@]}" --test-data "${i}" --joint "${j}" --n-predict "${k}" --scaler "${s}" -d "${OUTPUT_BASE}"/SET-B_joint-"$j"_n-predict-"$k"_scaler-"$s"-"${TEST_TRAJ_LABELS[$ii]}" -e png -s -x)
        echo "${cmd[@]}" >>"$LOG"
        "${cmd[@]}" >>"$LOG" 2>&1
      done
    done
  ...
  for j in "${JOINT[@]}"; do
    for k in "${N_PREDICT[@]}"; do
      for s in "${SCALER[@]}"; do
        ...
        cmd=(pdm run ./apps/predict_ctrl_input_linear.py --train-data "${TRAIN_DATA_SET_C[@]}" --test-data "${i}" --joint "${j}" --n-predict "${k}" --scaler "${s}" -d "${OUTPUT_BASE}"/SET-C_joint-"$j"_n-predict-"$k"_scaler-"$s"-"${TEST_TRAJ_LABELS[$ii]}" -e png -s -x)
        echo "${cmd[@]}" >>"$LOG"
        "${cmd[@]}" >>"$LOG" 2>&1
      done
    done
