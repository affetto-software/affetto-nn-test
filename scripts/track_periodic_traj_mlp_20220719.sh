...
OUTPUT_BASE="$HOME/Dropbox/develop/data/20220808_2/track_periodic_traj_mlp"
...
function do_all(){
  ...
  cmd=(pdm run ./apps/track_periodic_traj_mlp.py --train-data "$@" --joint 5 --output "data.csv" --time-duration 10 --n-predict 10 --n-ctrl-period 1 --ctrl mlp)
  echo "${cmd[@]}"
  "${cmd[@]}"
  for j in "${JOINT[@]}"; do
        ...
        for t in "${TRAJECTORY[@]}"; do
          for a in "${AMPLITUDE[@]}"; do
            for p in "${PERIOD[@]}"; do
              ...
              cmd=(pdm run ./apps/track_periodic_traj_mlp.py --train-data "$@" --joint "${j}" --n-predict "${k}" --n-ctrl-period 1 --scaler "${s}" --output "${output}" --trajectory "${t}" --time-duration "$DURATION" --amplitude "${a}" --period "${p}" --ctrl mlp --model "model.joblib")
              echo "${cmd[@]}" >>"$LOG"
              "${cmd[@]}"
